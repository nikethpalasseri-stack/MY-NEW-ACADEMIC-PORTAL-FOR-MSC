...
const AcademicCalendar: React.FC<AcademicCalendarProps> = ({ events }) => {
  ...
  return (
    <div className="animate-slide-up space-y-10">
          ...
          <div className="flex items-center gap-3">
             ...
             {today.getMonth() === selectedMonth && today.getFullYear() === selectedYear && (
               <span className="flex items-center gap-2 px-4 py-1.5 bg-emerald-500/10 text-emerald-600 rounded-full text-[9px] font-black uppercase tracking-widest border border-emerald-500/20 animate-pulse">
                 <div className="w-1.5 h-1.5 rounded-full bg-emerald-500"></div> Current Month Active
               </span>
             )}
          </div>
          <h2 className="text-5xl font-black text-slate-900 tracking-tighter flex items-center gap-4">
            <CalendarDays className="w-8 h-8 text-rose-600 drop-shadow-lg" />
            Academic Calendar
          </h2>
          <p className="text-slate-400 font-medium ml-1">AY 2025 - 2026 | Semester Roadmap</p>
        </div>

        <div className="flex items-center bg-white border border-slate-200 rounded-[2.5rem] p-1.5 shadow-xl">
          <button 
            onClick={() => setMscOnly(!mscOnly)}
            className={`flex items-center gap-3 px-6 py-3 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${mscOnly ? 'bg-rose-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-800'}`}
          >
      ...
      <div className="flex items-center gap-4 bg-slate-100/50 p-1.5 rounded-[2.5rem] border border-slate-200 overflow-x-auto no-scrollbar scroll-smooth">
        {availableMonths.map((m, idx) => (
          <button
            key={idx}
            onClick={() => { setSelectedMonth(m.month); setSelectedYear(m.year); }}
            className={`flex-shrink-0 px-6 py-3 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${selectedMonth === m.month && selectedYear === m.year ? 'bg-white text-rose-600 shadow-md border border-rose-100' : 'text-slate-400 hover:text-slate-800'}`}
          >
          ...
          {filteredEvents.length > 0 ? (
            filteredEvents.map((event, idx) => {
              ...
              return (
                <div 
                  key={idx} 
                  className={`group flex items-center gap-8 bg-white p-8 rounded-[2.5rem] border transition-all duration-500 ${
                    isToday 
                      ? 'border-rose-500 ring-2 ring-rose-500/20 shadow-2xl shadow-rose-500/10 scale-[1.02]' 
                      : 'border-slate-100 shadow-sm hover:shadow-xl'
                  }`}
                >
                  <div className={`flex flex-col items-center justify-center min-w-[60px] border-r pr-8 ${isToday ? 'border-rose-100' : 'border-slate-100'}`}>
                    <span className={`text-xl font-black tracking-tighter ${isToday ? 'text-rose-600' : 'text-slate-800'}`}>
                      {dateObj.getDate()}
                    </span>
                    <span className={`text-[9px] font-bold uppercase tracking-widest ${isToday ? 'text-rose-400' : 'text-slate-400'}`}>
                      {event.day}
                    </span>
                  </div>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest ${style.bg} ${style.text} border ${style.border}`}>
                        {style.label}
                      </span>
                      {isToday && (
                        <span className="px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest bg-rose-600 text-white animate-pulse">
                          Today
                        </span>
                      )}
                    </div>
                    <h4 className={`text-lg font-black uppercase tracking-tighter leading-tight transition-colors ${isToday ? 'text-rose-600' : 'text-slate-800 group-hover:text-rose-600'}`}>
                      {event.particulars}
                    </h4>
                  </div>

                  <div className={`p-3 rounded-xl shadow-sm transition-transform duration-500 group-hover:scale-110 ${isToday ? 'bg-rose-600 text-white' : `${style.bg} ${style.text}`}`}>
                    {style.icon}
                  </div>
                </div>
              );
            ...
            <div className="bg-white p-24 rounded-[3rem] border border-slate-100 text-center space-y-6">
